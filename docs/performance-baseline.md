...
| 前端 1000 行渲染、筛选、排序、选择及检索到单行 | 全流程 < 5 秒，单次 React commit < 1 秒 | 测试体约 2.76 秒 |
...
这些门槛是首版回归警戒线，不是跨机器的绝对性能承诺。测试使用真实 SQLite 索引和真实临时目录；前端测试覆盖列表渲染、勾选反馈、检索与结果收敛。重复检查在 Tauri 阻塞任务线程中执行，不占用浏览器主线程。
